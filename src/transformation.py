...
def limpiezas_generales_reservations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convertimos fechas de entrada (check-in)
    ...
    # Calculamos duración total de la estancia
    df["total_nights"] = df["no_of_weekend_nights"] + df["no_of_week_nights"]

    # Calculamos fecha de salida (check-out)
    df["check_out"] = df["check_in"] + df["total_nights"].apply(lambda x: timedelta(days=int(x)))

    # Convertimos campos numéricos importantes
    columnas_float = [
        "avg_price_per_room", "no_of_weekend_nights", "no_of_week_nights",
        "total_nights"
    ]
    ...
    # Eliminamos registros con check_in nulo (por errores de fecha)
